...
const UploadVideo = ({navigation, route}) => {
  ...
  /////////////redux states///////
  const {video, links, id, thumbnails, linksicon} = useSelector(
  ...
  const imagehandleUpload = uploadimage => {
    ...
    fetch('https://api.cloudinary.com/v1_1/mtechub/upload', {
      ...
      .then(data => {
        ...
        setImages(data.url);
        setSelectimages([...Selectimages, data.url]);
      })
      .catch(err => {
  ...
  const deleteImage = async item => {
    ...
    const data = new FormData();
    data.append('public_id', item.public_id);
    data.append('upload_preset', 'nrrfyy0m');
    data.append('cloud_name', 'mtechub');
    ...
    fetch('https://api.cloudinary.com/v1_1/mtechub/:image/destroy', {
      ...
      .then(data => {
        console.log('data here:', data);
        setImages(data.url);
        setSelectimages([...Selectimages, data]);
      })
      .catch(err => {
        alert('error while uploading');
      });
  ...
  const documenthandleUpload = uploadimage => {
    ...
    fetch('https://api.cloudinary.com/v1_1/mtechub/upload', {
      ...
      .then(data => {
        ...
        setSelectDocs([...Selectdoc, data.url]);
      })
      .catch(err => {
        alert('error while uploading');
      });
  ...
  const UpdateUserProfileVideo = () => {
    ...
    axios({
      ...
      .then(async function (response) {
        ...
        if (
          response.data === 'Profile Video Exists either Delete it or Update it'
        ) {
          setloading(0);
          setdisable(0);
  ...
  const GetProfileData = async () => {
    ...
    axios({
      method: 'GET',
      url: BASE_URL + 'user/get-user?_id=63172cd43a4b0aa534847c65',
      // + user,
    })
      .then(function (response) {
        console.log('response', JSON.stringify(response.data));
        ...
        setSelectDocs(response.data.profileVideoId.pdf);
      })
      .catch(function (error) {
        console.log('error', error);
      });
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView
        ...
        <View style={[Inputstyles.inputview, {marginTop: hp(0)}]}>
              ...
              <View
                ...
                <View
                  ...
                  <Image
                    ...
                    resizeMode="contain"
                  />
                  <Text style={[styles.orangetext, {marginLeft: wp(3)}]}>
                    {item.split('/')[7]}
                  </Text>
                </View>
          ...
          <TouchableOpacity
            onPress={() => {
              setlinksview(true), reflinkddRBSheet.current.open();
            }}>
            <View
              style={{
                flexDirection: 'row',
        ...
        <AddLinks
          ...
          onClose={() => {
            reflinkddRBSheet.current.close(), props.refRBSheet.current.close();
          }}
